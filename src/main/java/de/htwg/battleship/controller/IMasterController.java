...
public interface IMasterController extends IObservable {
     ...
     * @param y y-Coordinate where to shoot. checks the winner automatically after each shot
     */
    ...
    /**
     * To get the current State of the game. uses the different states in the util Package
     *
    ...
    /**
     * Method to set the whole players profile. Which players name and id is set is addicted to the current state of the
     * game. This method should be used in the UIs where an ID is given by a specific login method that all users
     * share.
     *
     * @param name of the player which will be displayed
     * @param id   of the player to identify if internally
     */
    void setPlayerProfile(String name, String id);

    /**
     * Method to set the players name. Which players name is set is addicted to the current state of the game
     *
    ...
    /**
     * Method to start a game or if the game is in the END-state to restart the game.
     */
    void startGame();
     ...
     * @return the new Map
     */
    Map<Integer, Set<Integer>> fillMap(IShip[] shipList, Map<Integer, Set<Integer>> map, int ships);

    /**
     * Method to get in the Options state, where you can set the GameMode. The Standard GameMode is the {@link
     * de.htwg.battleship.util.GameMode#NORMAL}
     */
    void configureGame();
    ...
    /**
     * Method to restore a whole game with a given state. TODO: make it only valid in specific states
     *
